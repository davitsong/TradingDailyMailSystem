...
def get_stock_data(market_type):
    try:
        if market_type == "morning":
            ...
            # 종가(현재가) 가져오기
            n_price = nasdaq['Close'].iloc[-1]
            s_price = sp500['Close'].iloc[-1]
            ...
            # 리턴 문자열에 가격(소수점 2자리) 추가
            return f"[미국 증시 기본 데이터] 나스닥: {n_price:,.2f} ({n_emoji}{n_chg:.2f}%), S&P 500: {s_price:,.2f} ({s_emoji}{s_chg:.2f}%)"
        else:
            kospi = yf.Ticker("^KS11").history(period="1d")
            kosdaq = yf.Ticker("^KQ11").history(period="1d")
            
            # 종가(현재가) 가져오기
            k_price = kospi['Close'].iloc[-1]
            kq_price = kosdaq['Close'].iloc[-1]
            ...
            # 리턴 문자열에 가격(소수점 2자리) 추가
            return f"[한국 증시 기본 데이터] 코스피: {k_price:,.2f} ({k_emoji}{k_chg:.2f}%), 코스닥: {kq_price:,.2f} ({kq_emoji}{kq_chg:.2f}%)"
...
def generate_report():
    ...
    if hour < 12:
        ...
        # 프롬프트 내에 '가격' 언급 규칙 추가
        prompt = f"""
        너는 최고의 금융 분석 송방원 에이전트야. 제공된 데이터({market_info})와 너의 최신 웹 검색/지식을 활용해서 아래 조건에 맞는 아침 리포트를 작성해줘.
        
        [🚨 가장 중요한 초강력 절대 규칙]
        - 보고서에 등장하는 '모든 수치(시장 지수, 개별 주식 종목)'는 반드시 현재 가격(또는 마감 가격)과 등락률을 함께 표기하고, 등락률 뒤에는 무조건 색상 이모지를 붙여야 해.
        - 가격이나 전일 대비 수치가 올랐거나 상승한 주식이면 무조건 빨간색 원형 이모지와 플러스 기호인 '0,000.00 달러 (🔴 +0.00%)' 형태로 적어줘.
        - 가격이나 전일 대비 수치가 내렸거나 하락한 주식이면 무조건 파란색 원형 이모지와 마이너스 기호인 '0,000.00 달러 (🔵 -0.00%)' 형태로 적어줘.
        - 특히 3번(급등 종목)과 4번(추천/주목 종목)에 작성하는 모든 주식은 이름 옆 괄호 안에 가격과 등락률을 '종목명(티커) [000.00달러, 🔴 +5.40%]' 혹은 '종목명(티커) [000.00달러, 🔵 -1.20%]' 형태로 100% 명시해야 해. 무슨 일이 있어도 예외는 없어!
        ...
        3. 오늘 새벽 미국 시장에서 많이 오른 급등 종목 3가지를 선정하고, 반드시 해당 기업의 '공식 이름(종목명)'과 '티커'를 명시하고 현재 가격과 상승률(🔴 +0.00%) 및 명확한 상승 이유를 적어줘.
        4. 오늘 밤 장에서 주목할 만한 미국 주식 3개를 추천하되, 반드시 해당 기업의 '공식 이름(종목명)'과 '티커'를 명확히 표기하고 예상 가격대와 등락률(🔴 또는 🔵 필수 표기) 및 구체적인 이유를 제시해줘.
        ...
        [🚨 가장 중요한 초강력 절대 규칙]
        - 보고서에 등장하는 '모든 수치(시장 지수, 개별 주식 종목)'는 반드시 현재 가격(또는 마감 가격)과 등락률을 함께 표기하고, 등락률 뒤에는 무조건 색상 이모지를 붙여야 해.
        - 가격이나 전일 대비 수치가 올랐거나 상승한 주식이면 무조건 빨간색 원형 이모지와 플러스 기호인 '0,000원 (🔴 +0.00%)' 형태로 적어줘.
        - 가격이나 전일 대비 수치가 내렸거나 하락한 주식이면 무조건 파란색 원형 이모지와 마이너스 기호인 '0,000원 (🔵 -0.00%)' 형태로 적어줘.
        - 특히 3번(급등 종목)과 4번(추천/주목 종목)에 작성하는 모든 주식은 이름 바로 옆에 가격과 등락률을 '종목명 [00,000원, 🔴 +5.40%]' 혹은 '종목명 [00,000원, 🔵 -1.20%]' 형태로 100% 명시해야 해. 무슨 일이 있어도 예외는 없어!
        ...
        3. 오늘 코스피/코스닥 시장에서 많이 오른 급등 종목 3가지를 선정하고, 반드시 해당 '기업 이름(종목명)'을 명시하고 현재 가격과 상승률(🔴 +0.00%) 및 명확한 상승 이유를 적어줘.
        4. 내일 장에서 주목할 만한 한국 주식 3개를 추천하되, 반드시 해당 '기업 이름(종목명)'을 명시하고 예상 가격대와 등락률(🔴 또는 🔵 필수 표기) 및 구체적인 이유를 제시해줘.
